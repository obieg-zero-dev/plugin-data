...
git clone https://github.com/obieg-zero-dev/plugin-data    # to repo
...
cd ../plugin-data && git add -A && git commit -m "..."
```
...
```bash
bash core-host/scripts/promote-to-dev.sh  plugin-data             # LOCAL -> DEV
bash core-host/scripts/promote-to-prod.sh plugin-data v1.2.3      # DEV   -> PROD + tag
```
